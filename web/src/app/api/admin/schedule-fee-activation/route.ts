...
export async function POST(request: NextRequest) {
  try {
    ...
    const mutation = `
      mutation ScheduleFeeActivation($network: String!, $vaultId: String!) {
        scheduleFeeActivation(network: $network, vaultId: $vaultId) { txId }
      }
    `;
    ...
    if (!json.data?.scheduleFeeActivation) {
      return NextResponse.json(
    ...
    return NextResponse.json(json.data.scheduleFeeActivation);
  } catch (error) {
    return NextResponse.json(
